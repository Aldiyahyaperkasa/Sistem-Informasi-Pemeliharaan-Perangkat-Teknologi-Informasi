<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Dashboard - Mazer Admin Dashboard</title>
    
    
    
    <link rel="shortcut icon" href="<?= base_url() ?>assets/compiled/svg/favicon.svg" type="image/x-icon">
    <link rel="shortcut icon" href="data:image/png;base64," type="image/png">
    


  <link rel="stylesheet" href="<?= base_url() ?>assets/compiled/css/app.css">
  <link rel="stylesheet" href="<?= base_url() ?>assets/compiled/css/app-dark.css">
  <link rel="stylesheet" href="<?= base_url() ?>assets/compiled/css/iconly.css">
</head>

<body>
    <script src="<?= base_url() ?>assets/static/js/initTheme.js"></script>
    <div id="app">
        <div id="sidebar">
            <div class="sidebar-wrapper active">
                <div class="sidebar-header position-relative">
                    <div class="d-flex justify-content-center align-items-center">
                        <div class="d-flex align-items-center">
                            <div class="logo">
                                <a href="<?= site_url('/Main/index') ?>">
                                    <img src="<?= base_url() ?>image/KMI.png" alt="Logo" style="height: 50px;">
                                </a>
                            </div>
                        </div>
                        <div class="sidebar-toggler x">
                            <a href="#" class="sidebar-hide d-xl-none d-block">
                                <i class="bi bi-x bi-middle"></i>
                            </a>
                        </div>
                    </div>
                </div>

                <?php 
                // Helper function to check if the current URL matches the given URL
                function isActive($uri) {
                    $currentPath = uri_string();
                    $uriPath = strtolower($uri);
                    $currentPath = strtolower($currentPath);

                    return strpos($currentPath, $uriPath) !== false ? 'active' : '';
                }

                function isSubmenuActive($uris) {
                    $currentUri = uri_string();
                    $currentPath = parse_url($currentUri, PHP_URL_PATH);
                    $currentPath = strtolower($currentPath); // Normalize path to lower case

                    foreach ($uris as $uri) {
                        if (strpos($currentPath, strtolower($uri)) === 0) {
                            return 'active';
                        }
                    }
                    return '';
                }
                ?>

                <div class="sidebar-menu">
                    <ul class="menu">
                        <li class="sidebar-title">Menu</li>

                        <li class="sidebar-item <?= isActive('Main') ?>">
                            <a href="<?= site_url('/Main/index') ?>" class="sidebar-link">
                                <i class="bi bi-grid-fill"></i>
                                <span>Dashboard</span>
                            </a>
                        </li>

                        <li class="sidebar-item <?= isActive('PenggunaController') ?>">
                            <a href="<?= site_url('/PenggunaController/index') ?>" class="sidebar-link">
                                <i class="bi bi-people"></i>
                                <span>Pengguna</span>
                            </a>
                        </li>

                        <li class="sidebar-item <?= isActive('PerangkatController') ?>">
                            <a href="<?= site_url('/PerangkatController/index') ?>" class="sidebar-link">
                                <i class="bi bi-hdd-fill"></i>
                                <span>Perangkat</span>
                            </a>
                        </li>

                        <li class="sidebar-item <?= isActive('KodeQrController') ?>">
                            <a href="<?= site_url('/KodeQrController/index') ?>" class="sidebar-link">
                                <i class="bi bi-qr-code-scan"></i>
                                <span>KODE QR</span>
                            </a>
                        </li>

                        <li class="sidebar-item has-sub <?= isSubmenuActive(['JadwalPemeliharaanController', 'LaporanController']) ?>">
                            <a href="#" class="sidebar-link">
                                <i class="bi bi-tools"></i>
                                <span>Pemeliharaan</span>
                            </a>
                            <ul class="submenu">
                                <li class="submenu-item <?= isActive('JadwalPemeliharaanController') ?>">
                                    <a href="<?= site_url('/JadwalPemeliharaanController/index') ?>">Jadwal Pemeliharaan</a>
                                </li>
                                <li class="submenu-item <?= isActive('LaporanController') ?>">
                                    <a href="<?= site_url('/LaporanController/laporan') ?>">Laporan Pemeliharaan</a>
                                </li>
                            </ul>
                        </li>

                        <li class="sidebar-item <?= isActive('AuthController') ?>">
                            <a href="<?= site_url('/AuthController/logout') ?>" class="sidebar-link">
                                <i class="bi bi-box-arrow-right"></i>
                                <span>Logout</span>
                            </a>
                        </li>
                    </ul>
                </div>
            </div>
        </div>

        <div id="main">
            <header class="mb-3">
                <a href="#" class="burger-btn d-block d-xl-none">
                    <i class="bi bi-justify fs-3"></i>
                </a>
            </header>            
            <div class="page-content">
                <section class="row">
                    <?= $this->renderSection('konten'); ?>
                </section>
            </div>           
        </div>
    </div>

    <script src="<?= base_url() ?>assets/static/js/components/dark.js"></script>
    <script src="<?= base_url() ?>assets/extensions/perfect-scrollbar/perfect-scrollbar.min.js"></script>
    <script src="<?= base_url() ?>assets/compiled/js/app.js"></script>
    <script src="<?= base_url() ?>assets/extensions/apexcharts/apexcharts.min.js"></script>
    <script src="<?= base_url() ?>assets/static/js/pages/dashboard.js"></script>
</body>

</html>